...
void sendData() {
  if(WiFi.status() == WL_CONNECTED) {
    ...
    if (!isnan(mlx.readObjectTempC())) {  // 온도 데이터 읽기
      objectTemp = mlx.readObjectTempC();
    } else {
      Serial.println("온도 센서에서 NaN 값 반환. 다시 시도합니다.");
    }


    // 전송할 데이터 JSON 형식으로 준비
    String jsonData = "{\"vibration\": " + String(vibrationCount) + ", \"heartRate\": " + String(heartRateValue) + ", \"temperature\": " + String(objectTemp, 2) + "}";
